...
export const useSyncStore = create<SyncState>()((set, get) => ({
  ...
  markSyncing: (id) => {
    set((state) => ({
      queue: state.queue.map((op) => (op.id === id ? { ...op, status: 'syncing' as const } : op)),
    }))
  },

  markSynced: (id) => {
    set((state) => ({
      queue: state.queue.map((op) => (op.id === id ? { ...op, status: 'synced' as const } : op)),
    }))
  },

  markFailed: (id) => {
    set((state) => ({
      queue: state.queue.map((op) => (op.id === id ? { ...op, status: 'failed' as const } : op)),
    }))
  },
